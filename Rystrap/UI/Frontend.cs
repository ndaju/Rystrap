...
using Rystrap.UI.Elements.Bootstrapper;
using Rystrap.UI.Elements.Dialogs;

namespace Rystrap.UI
{
    static class Frontend
    {
        ...
        private static IBootstrapperDialog GetCustomBootstrapper()
        {
            ...
            catch (Exception ex)
            {
                ...
                if (!App.LaunchSettings.QuietFlag.Active)
                    ShowMessageBox(string.Format(Strings.CustomTheme_Errors_SetupFailed, ex.Message, "Rystrap"), MessageBoxImage.Error); // NOTE: Rystrap is the theme name
        ...
        public static void ShowBalloonTip(string title, string message, System.Windows.Forms.ToolTipIcon icon = System.Windows.Forms.ToolTipIcon.None, int timeout = 5)
        {
            ...
            {
                Icon = Properties.Resources.IconRystrap,
