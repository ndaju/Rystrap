...
using Rystrap.RobloxInterfaces;
using Rystrap.UI.Elements.Bootstrapper.Base;
...
namespace Rystrap
{
    public class Bootstrapper
    {
        ...
        public string BackgroundUpdaterMutexName => $"Rystrap-BackgroundUpdater-{_launchMode}";

        public string MutexNamePrefix { get; set; } = "Rystrap-Bootstrapper";
        ...
        private async Task<bool> CheckForUpdates()
        {
            ...
            {
                App.Logger.WriteLine(LOG_IDENT, $"More than one Rystrap instance running, aborting update check");
                return false;
            }
            ...
            SetStatus(Strings.Bootstrapper_Status_UpgradingRystrap);

            try
            {
#if DEBUG_UPDATER
                string downloadLocation = Path.Combine(Paths.TempUpdates, "Rystrap.exe");
